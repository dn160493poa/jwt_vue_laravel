  ...
  methods: {
    login: function login() {
      ...
      axios.post('/api/auth/login', {
        ...
        localStorage.setItem('access_token', res.data.access_token); //alternative is - document.cookie = `access_token = ${res.data.access_token}`

        _this.$router.push({
          name: 'user.personal'
        });
      })["catch"](function (error) {
        _this.error = error.response.error;
...
var render = function () {
  ...
  return _c("div", { staticClass: "w-25" }, [
    ...
    _vm._v(" "),
    _vm.error
      ? _c("div", { staticClass: "text-danger pt-2" }, [
